...
class GN6_Errors():
	...
	TEST_OK = 'test_ok'
 	...
 	_descripcions = {
 		BAD_REQUEST : _('La crida al Gestor de Serveis no es correcte')
 		# El sol·licitant no existeix
 		,USER_NOT_FOUND : _("No s'ha trobat el teu usuari al Gestor de Serveis.")
 		# El sol·licitant no té rol de sol·licitant de tiquets en el domini
 		,WRONG_PERMISSION : _("No tens permissos per crear tiquets al Gestor de Serveis.")
 	...
 	def getDescription(self, code):
 		...
 		# No coneixem el codi d'error
 			...
 			return _('Hi ha hagut un problema inesperat amb la petició al Gestor')
...
class GN6_GestioTiquets():
	...
	# Gravetat del tiquet
	_dic_gravetats = {'baixa':'GRAVETAT_BAIXA', 'mitja':'GRAVETAT_MITJA'
						, 'alta':'GRAVETAT_ALTA'}
	# Proces i proces origen
	_dic_processos = {'aus':'PROCES_AUS', 'rin':'PROCES_RIN', 'pti':'PROCES_PTI'
						, 'ads':'PROCES_ADS', 'adm':'PROCES_ADM'}
	# Estat
